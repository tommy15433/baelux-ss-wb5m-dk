...
static void EE_Reset( EEE_var_t* pv, uint32_t address, uint8_t nb_pages );
static int EE_Recovery( EEE_var_t* pv );
static int EE_Transfer( EEE_var_t* pv, uint16_t addr, uint32_t page );
static int EE_WriteEl( EEE_var_t* pv, uint16_t addr, uint32_t data );
static int EE_ReadEl( const EEE_var_t* pv,
                      uint16_t addr, uint32_t* data, uint32_t page );
static int EE_SetState( const EEE_var_t* pv, uint32_t page, uint32_t state );
static uint32_t EE_GetState( const EEE_var_t* pv, uint32_t page );
...
EEE_var_t EEE_var[CFG_EE_BANK1_SIZE ? 2 : 1];

/*****************************************************************************/

int EEE_Init( int format, uint32_t base_address )
{
  int status;
  ...
  if ( format )
  {
    ...
    total_nb_pages =
      2 * (EEE_var[0].nb_pages + (CFG_EE_BANK1_SIZE ? EEE_var[1].nb_pages : 0));

//    if ( HW_FLASH_Erase( EE_FLASH_PAGE( EEE_var, 0 ), total_nb_pages, 0 ) != 0 )
//    {
//      return EE_ERASE_ERROR;
//    }

    if ( FD_EraseSectors( EE_FLASH_PAGE( EEE_var, 0 ), total_nb_pages) != 0 )
    {
      return EEE_ERASE_ERROR;
    }

    /* Set first page of each pool in ACTIVE State */
    status = EE_SetState( &EEE_var[0], 0, EE_STATE_ACTIVE );

    if ( CFG_EE_BANK1_SIZE && (status == EEE_OK) )
    {
      status = EE_SetState( &EEE_var[1], 0, EE_STATE_ACTIVE );
    }
  ...
  status = EE_Recovery( &EEE_var[0] );

  if ( CFG_EE_BANK1_SIZE && (status == EEE_OK) )
  {
    status = EE_Recovery( &EEE_var[1] );
  }
...
int EEE_Read( int bank, uint16_t addr, uint32_t* data )
{
  EEE_var_t *pv = &EEE_var[CFG_EE_BANK1_SIZE && bank];;
...
int EEE_Write( int bank, uint16_t addr, uint32_t data )
{
  EEE_var_t *pv = &EEE_var[CFG_EE_BANK1_SIZE && bank];;
  uint32_t page;
  ...
  {
    return EEE_STATE_ERROR;
  }

  /* Mark the ERASED page at RECEIVE state */
  if ( EE_SetState( pv, page, EE_STATE_RECEIVE ) != EEE_OK )
  {
    return EEE_WRITE_ERROR;
  }
  ...
  if ( EE_WriteEl( pv, addr, data ) != EEE_OK )
  {
    return EEE_WRITE_ERROR;
  }
  ...
  if ( EE_Transfer( pv, addr, page ) != EEE_OK )
  {
    return EEE_WRITE_ERROR;
  }
...
int EEE_Clean( int bank, int interrupt )
{
  EEE_var_t *pv = &EEE_var[CFG_EE_BANK1_SIZE && bank];
  uint32_t page;
...
void EEE_Dump( int bank, uint16_t addr, uint32_t* data, uint16_t size )
{
  EEE_var_t *pv = &EEE_var[CFG_EE_BANK1_SIZE && bank];;
    ...
    for ( page = 0; page < 2UL * pv->nb_pages; page++ )
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if ( EE_SetState( pv, page - 1, EE_STATE_VALID ) != EEE_OK )
          {
            return EEE_WRITE_ERROR;
          }
        }
      ...
      if ( state == EE_STATE_RECEIVE )
      {
        ...
        if ( EE_Transfer( pv, EE_TAG, page ) != EEE_OK )
        {
          return EEE_WRITE_ERROR;
        }
      }
...
static int EE_Transfer( EEE_var_t* pv, uint16_t addr, uint32_t page )
{
  ...
  if ( addr != EE_TAG )
  {
    ...
    while ( 1 )
    {
      ...
      if ( (state == EE_STATE_ACTIVE) || (state == EE_STATE_VALID) )
      {
        ...
        if ( EE_SetState( pv, page, EE_STATE_ERASING ) != EEE_OK )
        {
          return EEE_WRITE_ERROR;
        }
      }
  ...
  for ( var = 0; var < EE_NB_MAX_ELT * pv->nb_pages; var++ )
  {
    ...
    if  ( (var != addr) &&
          ((addr != EE_TAG) ||
           (EE_ReadEl( pv, var, &data, pv->current_write_page ) != EEE_OK)) )
    {
      /* Read the last variable update */
      if ( EE_ReadEl( pv, var, &data, last_page ) == EEE_OK )
      {
        EE_DBG( EE_7 );

        /* In case variable corresponding to the virtual address was found,
           copy the variable to the new active page */
        if ( EE_WriteEl( pv, var, data ) != EEE_OK )
        {
          return EEE_WRITE_ERROR;
        }
      }
...
static int EE_WriteEl( EEE_var_t* pv, uint16_t addr, uint32_t data )
{
  ...
  if ( pv->next_write_offset >= HW_FLASH_PAGE_SIZE )
  {
    ...
    if ( EE_SetState( pv, page + 1, EE_GetState( pv, page ) ) != EEE_OK )
    {
      return EEE_WRITE_ERROR;
    }
    ...
    if ( EE_SetState( pv, page, EE_STATE_VALID ) != EEE_OK )
    {
      return EEE_WRITE_ERROR;
    }
