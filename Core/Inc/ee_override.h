 ...
 *      Author: bosun
 *
 *  when using zigbee nvm, default emulated eeprom driver code provided by ST does not work.
 *  but ee driver from Sample from Zigbee_OnOff_Coord_NVM works.
 *  so override those library with ee_override.c/h.
 *  default driver is located ay Middlewares/ST/STM32_SPAN/zigbee/platform/
 *
 */
...
#ifndef EE_OVERRIDE_H__
#define EE_OVERRIDE_H__
...
{
  EEE_OK = 0,
  EEE_NOT_FOUND,     /* read data is not found in flash */
  EEE_CLEAN_NEEDED,  /* data is written but a "clean" is needed */
  EEE_ERASE_ERROR,   /* an error occurs during flash erase */
  EEE_WRITE_ERROR,   /* an error occurs during flash write */
  EEE_STATE_ERROR    /* state of flash is incoherent (needs clean or format) */
};
...
extern int EEE_Init( int format,
...
extern int EEE_Read( int bank, uint16_t addr, uint32_t* data );
...
extern int EEE_Write( int bank, uint16_t addr, uint32_t data );
...
extern int EEE_Clean( int bank, int interrupt );
...
extern void EEE_Dump( int bank, uint16_t addr, uint32_t* data, uint16_t size );
